...
public class AccountAccess extends javax.swing.JFrame {
    ...
    private JSONArray brainMasters;
    ...
    private void initComponents() {
        ...
        find4.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                find4MouseClicked(evt);
            }
        });
    ...
    private void find4MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_find4MouseClicked
        // TODO add your handling code here:
        find4.setText("");
    }//GEN-LAST:event_find4MouseClicked
    ...
    private void loadAccounts() {
        try (FileReader reader = new FileReader(filepath)) {
            ...
            brainMasters = (JSONArray) users.get("brain");
            ...
            for (Object obj : braineers) {
                ...
                model.addRow(new Object[]{user.get("role"), user.get("username"), user.get("password"), user.get("points")});
            }

            // Load Brain Masters
            for (Object obj : brainMasters) {
                JSONObject user = (JSONObject) obj;
                model.addRow(new Object[]{user.get("role"), user.get("username"), user.get("password"), ""});
            }
        } catch (Exception e) {
    ...
    private void addLiveSearch() {
        find4.getDocument().addDocumentListener(new DocumentListener() {
            ...
            private void liveSearch() {
                ...
                filteredAccounts.addAll((Collection<? extends JSONObject>) brainMasters.stream()
                        .filter(obj -> ((JSONObject) obj).get("username").toString().toLowerCase().contains(searchTerm))
                        .collect(Collectors.toList()));
                ...
                for (Object obj : filteredAccounts) {
                    JSONObject user = (JSONObject) obj;
                    model.addRow(new Object[]{user.get("role"), user.get("username"), user.get("password"), user.getOrDefault("points", "")});
                }
            }
        });
    }
    ...
    private void sortAccounts() {
        // Sort Braineers by Points in descending order
        List<JSONObject> sortedBraineers = (List<JSONObject>) braineers.stream()
                .sorted(Comparator.comparingInt(obj -> -Integer.parseInt(((JSONObject) obj).get("points").toString())))
                .collect(Collectors.toList());

        DefaultTableModel model = (DefaultTableModel) leaderboardTable1.getModel();
        model.setRowCount(0);

        // Load sorted Braineers
        for (Object obj : sortedBraineers) {
            JSONObject user = (JSONObject) obj;
            model.addRow(new Object[]{user.get("role"), user.get("username"), user.get("password"), user.get("points")});
        }

        // Load Brain Masters (unsorted)
        for (Object obj : brainMasters) {
            JSONObject user = (JSONObject) obj;
            model.addRow(new Object[]{user.get("role"), user.get("username"), user.get("password"), ""});
        }

        JOptionPane.showMessageDialog(this, "Accounts sorted by points!", "Success", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void deactivateAccount() {
        ...
        String username = leaderboardTable1.getValueAt(selectedRow, 1).toString();
        for (Object obj : braineers) {
            JSONObject user = (JSONObject) obj;
            if (user.get("username").equals(username)) {
                user.put("status", "Deactivated");
                break;
            }
        }
    ...
    private void deleteAccount() {
        ...
        brainMasters.removeIf(obj -> ((JSONObject) obj).get("username").equals(username));

        try (FileWriter writer = new FileWriter(filepath)) {
            JSONObject data = new JSONObject();
            JSONObject users = new JSONObject();
            users.put("brainer", braineers);
            users.put("brain", brainMasters);
